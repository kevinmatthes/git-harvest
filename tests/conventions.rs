...
//! exactly one space after a semicolon, and a category opening every commit
//! subject.  The prose rules were enforced by two Python scripts until
...
/// is generated, and `THIRDPARTY.md` and `debian/copyright` are dependency
/// licence text harvested by the build script — none of either is this
/// project's to spell or wrap; `copyright`'s DEP-5 field lines are folded to
...
///
/// The subjects that matter are `main`'s squash commits, not the
/// `Bucket ::= entry` commits of whatever branch the check runs from:  a
/// branch's commits become one `[Category]` subject only when it merges, and
/// the pull-request-title workflow already guards that at merge time.  So the
/// walk is `{tag}..origin/main`, not `..HEAD`.
///
